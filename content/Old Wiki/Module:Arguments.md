\-- This module provides easy processing of arguments passed to
Scribunto from \-- #invoke. It is intended for use by other Lua modules,
and should not be \-- called from #invoke directly.
...
local translate_mt = { \_\_index = function(t, k) return k end }
...
`   --`[`-- Set up argument translation. --`](--_Set_up_argument_translation._-- "-- Set up argument translation. --"){.wikilink}\
...
`   --`[`-- Generate the tidyVal function. If it has been specified by the user, we -- use that; if not, we choose one of four functions depending on the -- options chosen. This is so that we don't have to call the options table -- every time the function is called. --`](--_Generate_the_tidyVal_function._If_it_has_been_specified_by_the_user,_we_--_use_that;_if_not,_we_choose_one_of_four_functions_depending_on_the_--_options_chosen._This_is_so_that_we_don't_have_to_call_the_options_table_--_every_time_the_function_is_called._-- "-- Generate the tidyVal function. If it has been specified by the user, we -- use that; if not, we choose one of four functions depending on the -- options chosen. This is so that we don't have to call the options table -- every time the function is called. --"){.wikilink}\
